...
public class ExecutorSupport {
    ...
    /**
     * from/to 파라미터 설명. 기간을 받는 실행기 6종이 전부 {@link #parsePeriodOrRecent}를 쓰므로
     * 날짜는 언제나 선택 항목인데, 스키마에 그 사실을 적은 실행기는 2종뿐이었다. 나머지는 날짜를
     * 필수처럼 제시해서 모델이 사용자에게 기간을 되물었다 — 시스템 프롬프트가 "기간은 되묻지 말라"고
     * 못 박아도 스키마가 이기는 자리다.
     *
     * <p>실행기마다 문구를 따로 쓰면 또 어긋나므로 상수 하나로 묶는다.
     */
    public static final String FROM_DESC =
            "조회 시작일 YYYY-MM-DD. 선택 항목 — 사용자가 기간을 말하지 않았으면 비워 둔다"
            + "(최근 " + DEFAULT_PERIOD_DAYS + "일이 자동 적용된다). 기간을 되묻지 말 것.";

    /** @see #FROM_DESC */
    public static final String TO_DESC =
            "조회 종료일 YYYY-MM-DD. 선택 항목 — 생략하면 오늘이 적용된다.";

    /**
     * target 계열 파라미터 설명 끝에 공통으로 붙이는 금지 문구.
     *
     * <p>스키마가 "한글 종목명"을 요구하면, 모델은 "삼성"처럼 종목명이 아닌 입력을 유효한 종목명으로
     * 완성해서 채운다("삼성전자"). 그러면 백엔드는 "삼성"을 본 적이 없어 모호성을 판정할 기회 자체가
     * 사라지고 되묻기가 영영 뜨지 않는다. 모델이 규칙을 어긴 게 아니라 규칙대로 한 것이므로
     * 규칙 쪽을 금지형으로 바꾼다.
     *
     * <p>시스템 프롬프트에도 같은 규칙이 있지만, 모델은 프롬프트보다 파라미터 스키마를 더 잘 따른다.
     * 여기가 1차 방어다.
     */
    public static final String VERBATIM_TARGET =
            " 사용자가 말한 표현을 그대로 넣는다. 회사명을 추측해 완성하지 말 것 —"
            + " '삼성'을 '삼성전자'로, '하이닉스'를 'SK하이닉스'로 바꾸지 않는다."
            + " 어느 종목인지 모호한지는 백엔드가 판정해 되묻는다.";
