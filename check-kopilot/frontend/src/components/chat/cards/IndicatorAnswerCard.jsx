...
export default function IndicatorAnswerCard({ message, tourTarget = false }) {
  const { title, period, metrics, chart, commentary, evidence, complianceBanner } =
    message;

  return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:p-5">
      {complianceBanner && (
        <div className="mb-3 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800 lg:text-sm">
          <AlertTriangle size={14} className="mt-0.5 shrink-0" />
          <p>{complianceBanner}</p>
        </div>
      )}

      <div className="mb-3">
        <h3 className="text-sm font-semibold text-slate-900 lg:text-base">{title}</h3>
        <p className="text-xs text-slate-400 lg:text-sm">{period}</p>
      </div>
