      ...
      <div className="flex flex-wrap gap-2">
        {message.candidates.map((c) => (
          <button
            key={c.code}
            onClick={() => onSelect?.(c)}
            className="rounded-full border border-slate-200 bg-white px-4 py-1.5 text-lg text-slate-700 transition-colors hover:border-accent-400 hover:bg-accent-50 hover:text-accent-700 lg:px-5 lg:py-2 lg:text-[19px]"
          >
            {c.name}
