...
| 문서 | 내용 |
|---|---|
| [spec.md](spec.md) | **설계 스펙** — 문제 정의, 지표 카탈로그 7종, 아키텍처, 화면·UX, 가드레일, 데모 시나리오 |
...
**스택** — Java 21 / Spring Boot 3.5.3 / Spring AI 1.0.0(OpenAI `gpt-4o`) / MySQL 8 / Redis 7 / React 19 + Vite + Recharts
...
- LLM은 Claude가 아니라 **OpenAI `gpt-4o`** (2026-07-26 전환). Spring AI의 `ChatModel` 추상화를 쓰므로 tool 디스패처·루프·SSE는 provider 중립이고, 교체 지점은 스타터 의존성·`application.yml`·모델 ID 세 곳뿐이다
...
- 지표 7종 중 6종이 `/stock/m001/hist_info` 하나로 커버된다 (`F15301`로 ETF 괴리율까지)

## 진행 상황

| Task | 상태 |
|---|---|
| 1~10, 12, 13. 백엔드 (스캐폴딩 · CHECK API · 캐시/폴백 · 도메인 · 실행기 7종 · 가이드 · 카드/xlsx · tool 디스패처 · 세션 · chat 루프) | 완료 |
| 11, 14, 15. 수요조사 · 평가셋 · Admin API | 진행 예정 |
